...
当前仓库**没有替主项目自动选择顶层开源许可证**。这意味着公开到 GitHub 并不自动授予他人复制、修改或再分发主项目代码的权利。

如果你希望正式开源，发布前应根据自己的授权意图选择并添加顶层 `LICENSE`，例如 MIT、Apache-2.0 或其他适合的许可证。第三方目录继续遵循其自带许可证。

## 贡献

提交修改前建议阅读 [CONTRIBUTING.md](CONTRIBUTING.md)。涉及安全问题或隐私泄露时，请优先按照 [SECURITY.md](SECURITY.md) 的方式处理，不要把真实个人信息直接贴进公开 Issue。
