...
DEFAULT_RESUME = PROJECT_ROOT / ".local" / "resume_facts.yaml"
DEFAULT_PROFILE = PROJECT_ROOT / ".local" / "profile.yaml"
DEFAULT_SCORING = PROJECT_ROOT / "config" / "scoring.yaml"
DEFAULT_ELIGIBILITY = PROJECT_ROOT / ".local" / "eligibility_policy.yaml"
...
class JobMatcher:
    ...
    def _validate_configuration(self) -> None:
        if nested_get(self.resume, "metadata", "onboarding_complete") is not True:
            raise ValueError(
                "User onboarding is incomplete. Follow START_HERE.md and run "
                "scripts/validate_profile.py before matching jobs."
            )
    ...
    def _build_candidate_levels(self) -> dict[str, float]:
        levels = {
            ...
            "3D Vision": 0.0,
            "Image reconstruction": 0.0,
            "Image enhancement": 0.0,
            "Video understanding": 0.0,
            "Recommendation": 0.0,
            "Speech algorithms": 0.0,
            "Embedded systems": 0.0,
            "FPGA": 0.0,
            "Radar algorithms": 0.0,
            "Integrated sensing and communication": 0.0,
            "Time-series modeling": 0.0,
            "Multimodal learning": 0.0,
        }

        # User-specific skills can be recorded in either skills.catalog or
        # skills.domains. Unknown skills stay zero; project names never imply a
        # skill by themselves.
        for section_name in ("catalog", "domains"):
            section = nested_get(self.resume, "skills", section_name, default={})
            if not isinstance(section, dict):
                continue
            for raw_name, spec in section.items():
                raw_level = spec.get("level") if isinstance(spec, dict) else spec
                value = LEVEL_VALUE.get(normalize(raw_level), 0.0)
                normalized_name = normalize(raw_name).replace("_", " ")
                for canonical in SKILL_ALIASES:
                    if normalize(canonical) == normalized_name:
                        levels[canonical] = max(levels.get(canonical, 0.0), value)
                        break
        return levels
    ...
    def _project_score_and_evidence(
        ...
        projects = nested_get(self.resume, "projects", default={})
        ...
        signal_map = {
            "communication_ai": COMM_SIGNALS,
            "medical_cv": {**MEDICAL_STRONG_SIGNALS, **CV_SIGNALS},
            "general_ai": GENERAL_SIGNALS,
        }[track]
        jd_text = normalize(combined)
        candidates: list[tuple[int, dict[str, str], list[str]]] = []

        for project_name, project in projects.items():
            project_text = normalize(" ".join([str(project_name), *flatten(project)]))
            overlaps: list[str] = []
            for canonical, aliases in SKILL_ALIASES.items():
                if first_present(jd_text, aliases) and first_present(project_text, aliases):
                    overlaps.append(canonical)

            explicit_terms: list[str] = []
            if isinstance(project, dict):
                for field in ("keywords", "topics", "skills", "methods", "tools"):
                    explicit_terms.extend(flatten(project.get(field, [])))
            for raw_term in explicit_terms:
                term = normalize(raw_term)
                if len(term) >= 2 and term_present(jd_text, term):
                    overlaps.append(str(raw_term))
            overlaps = stable_unique(overlaps)[:8]
            if not overlaps:
                continue

            project_track = signal_score(project_text, signal_map)
            jd_track = signal_score(jd_text, signal_map)
            evidence_type = "direct" if project_track > 0 and jd_track > 0 else "transferable"
            score = min(20, 4 + 3 * len(overlaps) + (4 if evidence_type == "direct" else 0))
            evidence = {
                "project": str(project_name),
                "evidence": "与 JD 重合的已确认关键词：" + "、".join(overlaps),
                "evidence_type": evidence_type,
            }
            transfer = overlaps if evidence_type == "transferable" else []
            candidates.append((score, evidence, transfer))

        if not candidates:
            return 0, [], []
        candidates.sort(key=lambda item: (-item[0], item[1]["project"]))
        selected = candidates[:3]
        best_score = selected[0][0]
        if len(selected) > 1:
            best_score = min(20, best_score + min(3, len(selected) - 1))
        evidence = [item[1] for item in selected]
        transferable = stable_unique(
            skill for item in selected for skill in item[2]
        )
        return best_score, evidence, transferable
    ...
    def _education_background_score(self, track: str, combined: str) -> int:
        ...
        candidate_major = normalize(nested_get(self.resume, "candidate", "major") or "")
        if candidate_major and candidate_major in text:
            score = 15
        if any(
            phrase in text
    ...
    def _eligibility(
        ...
        expected_graduation = str(
            nested_get(self.resume, "candidate", "expected_graduation") or ""
        )
        year_match = re.match(r"(20\d{2})", expected_graduation)
        candidate_year = int(year_match.group(1)) if year_match else None
        candidate_degree = normalize(
            nested_get(self.resume, "candidate", "degree") or ""
        )
        candidate_major = str(nested_get(self.resume, "candidate", "major") or "")
        candidate_experience = nested_get(
            self.resume, "candidate", "fulltime_experience_years"
        )
        try:
            candidate_experience_years = float(candidate_experience)
        ...
        cohort_match = bool(
            candidate_year
            and (
                re.search(rf"(?:{candidate_year}|{str(candidate_year)[-2:]})\s*届", text)
                or re.search(
                    rf"{candidate_year}.{{0,12}}(?:校招|校园招聘|graduate|campus)", text
                )
                or normalize(job.get("graduation_cohort"))
                in {
                    str(candidate_year),
                    f"{candidate_year}届",
                    f"{str(candidate_year)[-2:]}届",
                }
            )
        )
        ...
        candidate_is_phd = any(term in candidate_degree for term in ("博士", "phd", "ph.d"))
        if (
            phd_only
            and not phd_flexible
            and not phd_preferred
            and not candidate_is_phd
        ) or any(
        ...
        explicit_other_cohort = bool(
            candidate_year is not None and cohorts and candidate_year not in cohorts
        )
        if explicit_other_cohort and (campus or internship):
            hard_conflicts.append(
                "毕业届次冲突：岗位明确面向 "
                + "/".join(str(year) for year in sorted(cohorts))
                + f" 届，不包含候选人的 {candidate_year} 届"
            )
        elif cohort_match:
            notes.append(f"毕业届次匹配：候选人预计 {expected_graduation} 毕业")
        elif not candidate_year:
            risks.append("候选人毕业年月尚未确认")
        elif campus:
            risks.append(f"毕业窗口未明确包含 {expected_graduation}，需人工确认")
        ...
        if years is not None and fulltime_experience and not (campus or internship):
            if candidate_experience_years is None:
                risks.append(
                    f"岗位要求至少 {years} 年经验；候选人的全职经验年限尚未结构化确认"
                )
            elif candidate_experience_years < years:
                hard_conflicts.append(
                    f"经验资格冲突：岗位要求至少 {years} 年，候选人确认的全职经验为 "
                    f"{candidate_experience_years:g} 年"
                )

        candidate_is_master_or_above = any(
            term in candidate_degree for term in ("硕士", "博士", "master", "phd", "ph.d")
        )
        if candidate_is_master_or_above and any(
            phrase in text
            for phrase in (
                "仅限本科且不接受硕士",
                "仅限本科生，不接受硕士",
        ...
        if exclusive_major and candidate_major and normalize(candidate_major) not in text:
            hard_conflicts.append(
                f"专业资格冲突：岗位明确限定专业，候选人专业为 {candidate_major}"
            )
        elif any(
            phrase in text
            ...
            risks.append(
                f"专业限制信息不完整，{candidate_major or '候选人专业'}是否在目录内需人工确认"
            )
        ...
        has_cet6 = any(term in credential_text for term in ("cet-6", "cet6", "英语六级"))
        if cet6 and not language_preferred and not has_cet6:
            hard_conflicts.append(
                "语言资格冲突：岗位明确要求 CET-6，但事实库未确认该证书"
            )
        ...
        if any(
            ...
            risks.append("岗位要求较高英语熟练度，需与候选人的已确认语言事实人工核对")
        ...
        internship_available = nested_get(
            self.eligibility_policy,
            "eligibility",
            "candidate",
            "internship_available",
        )
        if internship and internship_available is True:
            notes.append("事实库确认可实习，但具体到岗日期和时长仍需用户确认")
        elif internship and internship_available is not True:
            risks.append("候选人的实习可用性尚未确认")
        if not (campus or internship) and not hard_conflicts:
            risks.append("岗位不是明确的校招/实习，申请窗口和经验资格需人工确认")
    ...
    def _role_preference(self, title: str, combined: str) -> str:
        text = normalize(f"{title} {combined}")
        configured = nested_get(self.profile, "target_roles", default={})
        if isinstance(configured, dict):
            for priority in ("P0", "P1", "P2"):
                roles = configured.get(priority, [])
                if not isinstance(roles, list):
                    continue
                for role in roles:
                    role_text = normalize(role)
                    if role_text and (role_text in text or normalize(title) in role_text):
                        return priority
        return "unclassified"
    ...
    def _preference_fit(
        ...
        company = normalize(job.get("company") or "")
        preferred_companies = nested_get(
            self.profile, "company_preferences", "preferred", default=[]
        )
        excluded_companies = nested_get(
            self.profile, "company_preferences", "excluded", default=[]
        )
        preferred_hit = any(
            normalize(item) and normalize(item) in company
        ...
        if excluded_hit:
            company_preference = "excluded_by_user"
            company_score = 20
        elif preferred_hit:
            company_preference = "preferred_by_user"
            company_score = 90
        else:
        ...
        cpp_value = self.candidate_levels.get("C++", 0.0)
        if cpp_value >= 0.75 or cpp_requirement in {"none", "bonus_only"}:
            cpp_label = "no_penalty"
            cpp_deduction = 0
        elif cpp_requirement == "preferred":
            cpp_label = "small_penalty"
            cpp_deduction = 4
        elif cpp_requirement == "core_required":
            cpp_label = "deprioritize_and_review"
            cpp_deduction = 12
        else:
            cpp_label = "heavy_low_priority"
            cpp_deduction = 20
        score = round(
        ...
        return {
            ...
            "cpp_penalty": cpp_label,
            "state_owned_penalty": "not_used",
            "cpp_requirement": cpp_requirement,
            "state_owned_or_research_institute": False,
            "high_domain_match_override": False,
            "evidence_mode": evidence_mode,
            "transfer_only": evidence_mode in {"none", "transferable_only"},
        }
    ...
    def _opportunity_priority(
        ...
        if eligibility["status"] == "uncertain" or grade == "C":
            return "P2"
        if preference["score"] < 50 or preference["role_preference"] == "P2":
            return "P2"
    ...
    def _matches_and_missing(
        ...
        if years and (candidate_years is None or candidate_years < years):
            known = "未结构化确认" if candidate_years is None else f"{candidate_years:g} 年"
            missing.append(f"岗位提到 {years} 年及以上经验；候选人已确认全职经验为 {known}")
        if any(
            phrase in normalize(combined)
            for phrase in (
                "legally authorized to work",
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = args.output_dir or (PROJECT_ROOT / "data" / "runtime" / "match_results")
